...
class fileReader {
public:
	fileReader(const std::string& sFileName,const std::string& sFilePath) { //take in user input regarding filepath
	...
	void getUniques(const int& minChars,const bool caseSens) { //get unique words longer than minchars
		std::string tempLine;
		std::string sWord;
	...
	void sortAndOutputMap(const std::string& sFileName,const std::string& sFilePath,const bool& bAccending){ //take map, flip + transfer to multimap sorted decendingly
...
private:
	...
	void loadFile(const std::string& sFileName,const std::string& sFilePath) {
	...
	void evalWord(std::string& sWord,const char splitDelim,const int& minChars) { //evaluate if input word should be recorded
	...
	void checkLenAndRecord(std::string& sWord,const int& minChars) {
	...
	std::vector<std::string> strToVec(std::string& sWord,const char& delim) { //take string, split by delin char, put resulting elements into vec
