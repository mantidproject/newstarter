...
typedef std::vector<std::shared_ptr<Shape>>
    vecShape; ///< To save typing in this implementation file
...
/**
 * \brief Selects all shapes in the given vector that satisfy the given
 * predicate
 *
 * \param shapes Vector of shared_ptr's to Shape objects, to select from
 * \param tester ShapeTester that determines if the given shape should be
 * selected or not
 * \returns Vector of selected shapes
 */
vecShape ShapeSorter::select(const vecShape &shapes,
                             const ShapeTester &tester) const {
  vecShape selected;
...
/**
 * \brief Sorts the shapes in the given vector according to the given
 * comparator,
 ...
 */
vecShape
ShapeSorter::sort(const vecShape &shapes,
                  bool predicate(const std::shared_ptr<Shape> &x,
                                 const std::shared_ptr<Shape> &y)) const {
 ...
 */
vecShape ShapeSorter::selectByType(const vecShape &shapes,
...
/**
 * \brief Selects all shapes in the given vector that have the given number of
 * sides
 *
 * \param shapes Vector of shared_ptr's to Shape objects, to select from
 ...
 * \returns Vector of shared_ptr's to matching Shapes
 */
vecShape ShapeSorter::selectBySides(const vecShape &shapes,
                                    const int numSides) const {
