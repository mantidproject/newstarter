...
/** General-purpose function for printing every shape which satisfies a given condition
 *
 * @param pred A lambda for which shapes should be printed
 *
 */
void ShapeSorter::printOnCondition(const std::function<bool (Shape *shape)> pred) const{
 ...
 *
 */
void ShapeSorter::printInOrder(const std::function<bool (Shape *s1, Shape *s2)> compare) const{
...
void ShapeSorter::shapesInAreaOrder() const{
  printInOrder([](Shape *s1, Shape *s2){return s1->getArea() > s2->getArea();});
}


/** Print shapes in descending order of their perimeter */
void ShapeSorter::shapesInPerimeterOrder() const{
  printInOrder([](Shape *s1, Shape *s2){return s1->getPerimeter() > s2->getPerimeter();});
}
